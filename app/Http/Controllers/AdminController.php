...
public function logout(){
	...
	{
		$data=DB::table('users')->join('datauser','datauser.user_id','=','users.id')->where('level','Pelanggan')->get();
		return view('page/user/user',['data'=>$data]);
	}
	public function pengguna()
	{
		$data=DB::table('users')->join('datauser','datauser.user_id','=','users.id')->where('level','Member')->get();
		return view('page/user/pengguna',['data'=>$data]);
	}
	public function validasiuser($id)
	{
		$user=User::where('users.id',$id)->get();
		$data=Data_sewa::with('nama_lapangan')->where('data_sewa.id_user',$id)->latest()->get();
	...
	{
		$cek=Jadwal::where('id_lap',$request->id_lap1 && $request->id_lap2)->where('tanggalmain',$request->tanggal1 && $request->tanggal2)->where('status','=','1')->first();
		...
		if ($cek) {
			if ($request->jam_mulai>=$cek->jam_mulai AND $request->jam_selesai<=$cek->jam_selesai) {
				return redirect()->back()->with('digunakanpb','-');
			}
			$sewa = new Data_sewa();
			$sewa -> id_user = $request -> id_user;
			$sewa -> lap_id = $request -> lap_id;
	...
	// 	$data=Pembayaran::with('data_sewa','user','nama_lapangan')->whereBetween('tanggal',[$tglawal, $tglakhir])->get();
	// 	$tgl=$tglawal;
	// 	$akhir=$tglakhir;
	// 	return view('page.laporan.pdf', compact('data','tgl','akhir'));
	// }
	...
	public function cetaklaporan(Request $request)
	{
		...
		if($tgl_mulai AND $tgl_selesai){
			$data = Pembayaran::with('data_sewa','user','nama_lapangan')->whereBetween('tanggal',[$tgl_mulai, $tgl_selesai])->get();
	...
	public function print(){
        $data=DB::table('data_sewa')->join('nama_lapangan','data_sewa.lap_id','=','nama_lapangan.id_lapangan')->join('jenis_lapangan','nama_lapangan.jenis_id','=','jenis_lapangan.id_jenis')->join('payment','data_sewa.id_payment','=','payment.id_payment')->join('pembayaran','pembayaran.sewa_id','=','data_sewa.id_sewa')->join('users','users.id','=','data_sewa.id_user')->join('datauser','datauser.user_id','=','users.id')->get();
	...
	public function diskon()
	{
		...
		return view('page/diskon/diskon',['data'=>$data]);
	}
